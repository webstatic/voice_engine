...
let inputPath = "./stage - md/stage 4/"
...
function textExclude(text) {
    return text.indexOf('///') == 0
        || text.indexOf('#') == 0
        || text.indexOf('<br>') == 0
}

function textCheckNonSentence(text) {
    return text.indexOf('stage') == 0
        || text.indexOf('ーーーー') == 0
        || text.indexOf('カンバセーション。') == 0
}
...
readAndSynthesisRecursive = function (filePath, finishCallBack) {
    ...
    async.eachSeries(textList, function (text, callback) {
        count++
        if (text !== undefined) {
            if (text == '' || textExclude(text)) {
                callback()
            } else {
                let textSpt = text.split("//")
                text = textSpt[0]
                if (text && text != '') {
                    if (!textCheckNonSentence(text) && textSpt.length != 2)
